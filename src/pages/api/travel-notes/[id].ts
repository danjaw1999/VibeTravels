...
export const GET: APIRoute = async ({ params, locals }) => {
  try {
    if (!params.id) {
      return new Response(
        JSON.stringify({ error: 'Travel note ID is required' }),
        {
          status: 400,
    ...
    if (!travelNote) {
      return new Response(
        JSON.stringify({ error: 'Travel note not found' }),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' }
        }
    ...
    if (!travelNote.is_public) {
      ...
      if (authError || !user?.id || user.id !== travelNote.user_id) {
        return new Response(
          JSON.stringify({ error: 'Authentication required to view this travel note' }),
          {
            status: 401,
            headers: { 'Content-Type': 'application/json' }
          }
...
export const PUT: APIRoute = async ({ request, params, locals }) => {
  try {
    ...
    if (authError || !user?.id) {
      return new Response(
        JSON.stringify({ error: 'Authentication required to update travel notes' }),
        { 
          status: 401,
          headers: { 'Content-Type': 'application/json' }
        }
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('Row level security')) {
        return new Response(
          JSON.stringify({ error: 'You can only update your own travel notes' }),
          {
            status: 403,
            headers: { 'Content-Type': 'application/json' }
          }
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  try {
    ...
    if (authError || !user?.id) {
      return new Response(
        JSON.stringify({ error: 'Authentication required to delete travel notes' }),
        { 
          status: 401,
          headers: { 'Content-Type': 'application/json' }
        }
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('Row level security')) {
        return new Response(
          JSON.stringify({ error: 'You can only delete your own travel notes' }),
          {
            status: 403,
            headers: { 'Content-Type': 'application/json' }
          }
