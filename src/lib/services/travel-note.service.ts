...
export class TravelNoteService {
  ...
  async listTravelNotes(query: TravelNoteQuery): Promise<{ data: TravelNoteDTO[]; total: number }> {
    ...
    let queryBuilder = this.supabase.from("travel_notes").select("*, attractions(*)", { count: "exact" });
  ...
  async deleteTravelNote(id: string, userId: string): Promise<void> {
    const { error } = await this.supabase.from("travel_notes").delete().eq("id", id).eq("user_id", userId);
  ...
  async addAttractions(
    ...
    const validatedAttractions = attractions.map((attraction) => createAttractionSchema.parse(attraction));

    // Insert attractions
    const { data, error } = await this.supabase.from("attractions").insert(
  ...
  private mapToDTO({
    ...
    updated_at,
    attractions,
  }: DBTravelNote & { attractions: DBAttraction[] }): TravelNoteDTO {
    return {
      id,
      user_id,
      name,
